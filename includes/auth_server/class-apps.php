...
Class VibeBP_oAuth_Apps{
	...
	public static function init(){
        ...
        if ( is_null( self::$instance ) )
            self::$instance = new VibeBP_oAuth_Apps();
	...
	function remove_update(){
		if(empty($_POST['update_actions']) || empty($_POST['id']) || !wp_verify_nonce('update_actions',$_POST['update_actions'])){
			echo _x('Security check failed','','vibe-bp-api');
			die();
		}
	...
	function send_updates(){
		...
		?>
		<a class="send_update_toggle button button-primary"><?php echo _x('Send New Update','send update button ','vibe-bp-api')?></a><br/>
		<div id="update_to_all_form">
			<a class="hide_update_form"><span class="dashicons dashicons-no"></span></a>
			<h4><?php echo _x('Send update to all app users.','','vibe-bp-api');?></h4>
			<?php wp_nonce_field('update_to_all','update_to_all');?>
			<textarea cols="60" rows="5" name="update_content" class="update_content" placeholder="<?php echo _x('Your update content.','contane label','vibe-bp-api')?>"></textarea><br/>
			<label class="min"><?php echo _x('Html tags supported.','contane label','vibe-bp-api')?></label><br/><br/>
			<a class="send_update button button-primary"><?php echo _x('Send Update','send update button ','vibe-bp-api')?></a>
		</div>
		<?php
		if(!empty($this->global_updates)){
			wp_nonce_field('update_actions','update_actions');
			echo '<h4>'._x('Sent Updates','','vibe-bp-api').'</h4>';
			echo '<table class="wp-list-table widefat fixed striped wp_list_test_links">
				<thead>
				<tr>
					<th scope="col" id="" class="manage-column column-name column-primary">'._x('Content','content','vibe-bp-api').'</th><th scope="col" id="description" class="manage-column column-description">'._x('Time','app table label','vibe-bp-api').'</th><th scope="col" id="client_id" class="manage-column column-client_id">'._x('Actions','app table label','vibe-bp-api').'</th>	</tr>
				</thead>
				...
				foreach($this->global_updates as $k=> $update){
					echo '<tr class="update_row">
						...
						<td class="actions column-actions"><a href="javascript:void(0)" data-id="'.$k.'" class="remove_update button">'._x('Remove','','vibe-bp-api').'</a></td></tr>';
			...
			echo '<div class="notice message is-dimissable"><p>'._x('No updates found.','','vibe-bp-api').'</p></div>';
		}
		...
		<script>
			...
			jQuery(document).ready(function($){
				...
				$('.remove_update').on('click',function(){
					if(confirm("<?php echo _x('Are sure you want to remove this update?','','vibe-bp-api');?>")){
						var $this = $(this);
						var init_text = $this.text();
						$this.text('<?php echo _x('Removing...','removing...','vibe-bp-api');?>')
						jQuery.ajax({
			              type: "POST",
			              url: ajaxurl,
				...
				$('.send_update').on('click',function(){
					...
					if(!$this.parent().find(".update_content").val()){
						alert("<?php echo _x('Please enter some content.','','vibe-bp-api');?>");
						return false;
					}
					var init_text = $this.text();
					$this.text('<?php echo _x('Sending...','sending...','vibe-bp-api');?>')
					jQuery.ajax({
		              type: "POST",
		              url: ajaxurl,
	...
	function new_app_form(){
		echo '<form method="post"><input type="submit" name="wplms_new_app" value="'._x('Create new app','new app button label','vibe-bp-api').'" class="button-primary">';
		...
		if(((isset($_POST['wplms_new_app']) && wp_verify_nonce($_POST['create_app_button'],'wplms_create_newapp')) || (isset($_POST['edit_app']))) && current_user_can('manage_options')){
			echo '<div class="card"><h2>'.(isset($_POST['edit_app'])?_x('Edit app','form title','vibe-bp-api'):_x('Create a new app','form title','vibe-bp-api')).'</h2><form method="post">';

			echo '<p><input type="text" name="app_name" class="regular-text" placeholder="'._x('App Name','wp admin form','vibe-bp-api').'" value="'.(isset($_POST['edit_app'])?$this->apps[$_POST['edit_app']]['name']:'').'"></p>';
			echo '<p><input type="text" name="app_description" class="regular-text" value="'.(isset($_POST['edit_app'])?$this->apps[$_POST['edit_app']]['description']:'').'"" placeholder="'._x('App Description','wp admin form','vibe-bp-api').'"></p>';
			echo '<p><input type="text" name="app_redirect_uri" class="regular-text" placeholder="'._x('Redirect URI','wp admin form','vibe-bp-api').'" value="'.(isset($_POST['edit_app'])?$this->apps[$_POST['edit_app']]['redirect_uri']:site_url()).'"></p>';
			echo '<p><input type="submit" name="wplms_add_app" value="'.(isset($_POST['edit_app'])?_x('Edit app','new app button label','vibe-bp-api'):_x('Create new app','new app button label','vibe-bp-api')).'" class="button-primary">&nbsp;<a class="button" onClick="">'.__('Close','vibe-bp-api').'</a></p>';
	...
	function handle_save(){
		...
		if(isset($_POST['wplms_add_app']) && wp_verify_nonce($_POST['create_app'],'wplms_create_newapp') && current_user_can('manage_options')){
			...
			if(empty($_POST['app_name']) || empty($_POST['app_description']) || empty($_POST['app_redirect_uri'])){
				$this->display_message(_x('Empty Name/Descript/Redirect URI','validation message','vibe-bp-api'),'error');
				return;
			}
			$new_app = array(
				'name' 			=>	$_POST['app_name'],
			...
			if(isset($_POST['edit_app_key'])){
				...
				$this->display_message(_x('App Updated successfully','success message','vibe-bp-api'),'updated');
			}else{
				$this->apps[] = $new_app;	
				$this->display_message(_x('App Created successfully','success message','vibe-bp-api'),'updated');
			}
	...
	function show_apps(){
		if(empty($this->apps) && empty($_POST)){
			$this->display_message(_x('No apps found ! Create a new App','app message','vibe-bp-api'),'updated');
			...
			echo '<table class="wp-list-table widefat fixed striped wp_list_test_links">
				<thead>
				<tr>
					<th scope="col" id="name" class="manage-column column-name column-primary">'._x('App Name','app table label','vibe-bp-api').'</th><th scope="col" id="description" class="manage-column column-description">'._x('App Description','app table label','vibe-bp-api').'</th><th scope="col" id="client_id" class="manage-column column-client_id">'._x('App ID','app table label','vibe-bp-api').'</th>	</tr>
				</thead>
						...
						<td class="name column-name">
							...
							<div class="row-actions">
								<form method="post" id="form_'.$app['app_id'].'" style="float:left">
									<span class="edit"><a onclick="document.getElementById(\'form_'.$app['app_id'].'\').submit();">'.__('Edit','vibe-bp-api').'</a> | </span>
									<input type="hidden" name="edit_app" value="'.$k.'" />
								</form>
								<form method="post" id="form_delete_'.$app['app_id'].'" style="float:left">	
									<span class="trash"><a class="submitdelete" onclick="confirm(\''. __('Are you sure you want to delete this app ?','vibe_customtypes').'\') && document.getElementById(\'form_delete_'.$app['app_id'].'\').submit();">'.__('Delete','vibe-bp-api').'</a> | </span>
									<input type="hidden" name="delete_app" value="'.$k.'" />
								</form>	
								<span class="view"><a  href="#TB_inline?width=400&height=100&inlineId=app_'.$app['app_id'].'"  class="thickbox">'.__('Show Secret','vibe-bp-api').'</a></span>
							</div>
						</td>
				...
				echo '</tbody><tfoot>
				<tr>
					<th scope="col" class="manage-column column-name column-primary">'._x('App Name','app table label','vibe-bp-api').'</th><th scope="col" class="manage-column column-description">'._x('App Description','app table label','vibe-bp-api').'</th><th scope="col" class="manage-column column-client_id">'._x('App ID','app table label','vibe-bp-api').'</th>	</tr>
				</tfoot>
	...
	function show_clients(){
        ...
        wp_enqueue_script('wplms-api-clients-select2-js',VIBE_PLUGIN_URL.'/vibe-bp-api/metaboxes/js/select2.min.js');
		wp_enqueue_style('wplms-api-clients-select2-css',VIBE_PLUGIN_URL.'/vibe-bp-api/metaboxes/css/select2.min.css');
		...
		if(!empty($_POST['clear_all_connected_clients']) && wp_verify_nonce($_POST['clear_all_connected_clients'],'clear_all_connected_clients')){
			...
			$this->display_message(_x('Connected clients deleted successfully','success message','vibe-bp-api'),'updated');
		...
		if(!empty($_POST['clear_all_expired_clients']) && wp_verify_nonce($_POST['clear_all_expired_clients'],'clear_all_expired_clients')){
			...
			$this->display_message(_x('Connected expired clients deleted successfully','success message','vibe-bp-api'),'updated');
		}
		...
		if(empty($this->clients)){
			echo '<div class="message"><p>'._x('No connected clients !','no clients found','vibe-bp-api').'</p></div>';
			return;
		}
		add_thickbox();
		//pagination :
		...
		$order_options = apply_filters('wplms_api_connected_cleints_order_options',array(
				'DESC'=>_x('Latest','','vibe-bp-api'),
				'ASC'=>_x('Oldest','','vibe-bp-api'),
		...
		<form id="api_username_orderby_form" method="get" action="?<?php echo $_SERVER['QUERY_STRING']; ?>">
			...
			<select name="wplms_api_clients_userid[]" id="wplms_api_clients_userid" class="selectusers_clients" data-placeholder="<?php echo __('Enter Student Usernames/Emails, separated by comma','vibe-bp-api');?>" multiple>
				<?php
			...
			<label><?php echo _x('Order by','search text api connected client','vibe-bp-api');?></label>
			...
			<input type="submit" class="button button-primary" value="<?php echo _x('Go','go button label','vibe-bp-api'); ?>">
		...
		echo '<table class="wp-list-table widefat fixed striped wp_list_test_links">
				<thead>
				<tr>
					<th scope="col" class="manage-column column-name column-primary">'._x('Client ID','app table label','vibe-bp-api').'</th><th scope="col" class="manage-column column-description">'._x('User','app table label','vibe-bp-api').'</th><th scope="col" class="manage-column column-client_id">'._x('Access Token','app table label','vibe-bp-api').'</th><th scope="col" id="client_id" class="manage-column column-client_id">'._x('Expires','app table label','vibe-bp-api').'</th>
						<th scope="col" id="device_id" class="manage-column column-device_id">'._x('Device Info','app table label','vibe-bp-api').'</th>
					<th>
						<i class="dashicons dashicons-email" title="'._x('Send private update','','vibe-bp-api').'"></i>/
						<i class="dashicons dashicons-visibility" title="'._x('View sent updates','','vibe-bp-api').'"></i></th><th>'._x('Remove','app table label','vibe-bp-api').'</th>	</tr>
				</thead>
		...
		foreach($this->clients as $client){
			if(!empty($client['client_id']) && !empty($client['user_id'])){
				...
				echo '<tr><td>'.$client['client_id'].'</td>
					...
					<td>
						<a class="send_update_notice" title="'._x('Send private update to this user','','vibe-bp-api').'"><i class="dashicons dashicons-email"></i></a>/
						<a class="thickbox view_recent_notice" title="'._x('View sent notices','','vibe-bp-api').'" href="#TB_inline?width=800&amp;inlineId=user_'.$client['user_id'].'"><i class="dashicons dashicons-visibility"></i></a>
						<div class="hide_message">
							...
							<a class="send_notice button">'.__('Send','vibe-bp-api').'</a>
						</div>
					</td>
					<td><form method="post"><input type="submit" value="'._x('Remove','app table label','vibe-bp-api').'" class="button" /><input type="hidden" name="token" value="'.$client['access_token'].'" />';
		...
		echo '</tbody><tfoot>
				<tr>
					<th scope="col" class="manage-column column-name column-primary">'._x('Client ID','app table label','vibe-bp-api').'</th><th scope="col" class="manage-column column-description">'._x('User','app table label','vibe-bp-api').'</th><th scope="col" class="manage-column column-client_id">'._x('Access Token','app table label','vibe-bp-api').'</th><th scope="col" id="client_id" class="manage-column column-client_id">'._x('Expires','app table label','vibe-bp-api').'</th><th scope="col" id="device_id" class="manage-column column-device_id">'._x('Device Info','app table label','vibe-bp-api').'</th><th>'._x('Remove','app table label','vibe-bp-api').'</th></tr>
				</tfoot>
		...
		echo		'<hr>'.__('Actions','vibe-bp-api').' : 
				<form class="clear_clients_form" method="post">
				<button type="submit" name="clear_all_connected_clients" value="1" class="clear_clients button button-primary" id="clear_all_connected_clients">'.__('Clear all','vibe-bp-api').'</button>
				'.wp_nonce_field('clear_all_connected_clients','clear_all_connected_clients').'
				</form>

				<form class="clear_clients_form" method="post">
				<button type="submit" name="clear_all_expired_clients" value="1"  class="clear_clients_expired button button-primary" id="clear_all_expired_clients">'.__('Clear all expired clients','vibe-bp-api').'</button>
				...
				jQuery(".clear_clients").on("click",function(event){
					if(confirm("'._x('Do you want to clear all clients ?','app notice','vibe-bp-api').'")){
				...
				jQuery(".clear_clients_expired").on("click",function(event){
					if(confirm("'._x('Do you want to clear all expired clients ?','app notice','vibe-bp-api').'")){
					    ...
					    $this.select2({
					        ...
					        language: {
					          inputTooShort: function() {
					            return '<?php echo _x('Please type atleast four characters','','vibe-bp-api');?>';
					          }
					        },
					        ajax: {
					            url: ajaxurl,
				...
				foreach ($all_users as $user_id) {
					...
					if(!empty($updates)){
						...
						echo '<h3> '.sprintf(_x('All Updates sent to %s','','vibe-bp-api'),bp_core_get_user_displayname($user_id)).'</h3><table class="wp-list-table widefat fixed striped wp_list_test_links">
							<thead>
							<tr>
								<th scope="col" id="" class="manage-column column-name column-primary">'._x('Content','content','vibe-bp-api').'</th><th scope="col" id="description" class="manage-column column-description">'._x('Time','app table label','vibe-bp-api').'</th>
								<th scope="col" id="client_id" class="manage-column column-client_id">'._x('Delivered','Delivered table label','vibe-bp-api').'</th>
								<th scope="col" id="client_id" class="manage-column column-client_id">'._x('Actions','app table label','vibe-bp-api').'</th>	
							</tr>
							</thead>

							<tbody id="the-list" data-wp-lists="list:wp_list_text_link">';
							foreach($updates as $k=> $update){
								$access_time = _x('Not delivered yet','','vibe-bp-api');
								...
								echo '<tr class="update_row">
									...
									<td class="actions column-actions"><a href="javascript:void(0)" data-id="'.$k.'" data-user_id="'.$user_id.'" class="remove_update_user button">'._x('Remove','','vibe-bp-api').'</a></td></tr>';
						...
						echo '<div class="notice update-nag message"><p>'._x('No updates found.','','vibe-bp-api').'</p></div>';
					}
